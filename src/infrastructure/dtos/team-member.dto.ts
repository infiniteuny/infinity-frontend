...
interface TeamMemberMembershipDto {
  id: string;
  user_id: string;
  team_id: string;
  role: string;
...
export interface TeamMemberDto {
  ...
  updated_at: string;
  major?: MajorDto;
  membership: TeamMemberMembershipDto;
}

export class TeamMemberMapper {
  public static fromDomainToDto(member: Partial<TeamMember>): Partial<TeamMemberDto> {
    return {
      id: member.id,
      name: member.name,
      ...
      links: member.links,
      start_date: member.startDate ? member.startDate.toISOString().split('T')[0] : null,
      end_date: member.endDate ? member.endDate.toISOString().split('T')[0] : null,
      ...
      membership: member.membership
        ? {
            ...
            team_id: member.membership.teamId,
  ...
  public static fromDtoToDomain(dto: TeamMemberDto): TeamMember {
    return new TeamMember(
      ...
      dto.major_id,
      dto.links,
      dto.start_date ? DateTime.fromISO(dto.start_date, { zone: 'UTC' }).toJSDate() : null,
      dto.end_date ? DateTime.fromISO(dto.end_date, { zone: 'UTC' }).toJSDate() : null,
      dto.is_member,
