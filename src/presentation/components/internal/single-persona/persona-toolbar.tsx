...
export function PersonaToolbar({ personaId, ref, methods }: OneOf<[ViewProps, FormProps]>) {
  ...
  if (personaId) {
    return (
      <Box className="ml-auto">
        {['update-persona'].some((p) => userPermissions.includes(p)) ? (
          <Button
            variant="filled"
            className="ml-4"
            aria-label="Edit persona"
            LinkComponent={Link}
            href={`/personas/${personaId}/edit`}
