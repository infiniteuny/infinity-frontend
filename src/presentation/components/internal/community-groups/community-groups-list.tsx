...
export function CommunityGroupsList({ initialCommunityGroups, initialPaginationOptions }: Props) {
  ...
  const handleDeleteClick = (communityGroupId: string, communityGroupName?: string) => {
    setSelectedCommunityGroupId(communityGroupId);
    setSelectedCommunityGroupName(communityGroupName || null);
    setOpenDeleteDialog(true);
  };

  const handleDeleteAccept = async () => {
    if (!selectedCommunityGroupId) {
      console.error('No community group selected for deletion');

      return;
    }

    const result = await deleteCommunityGroup.execute(selectedCommunityGroupId);
    match(result, {
      onRight: () => {
        setRows((prevRows) => prevRows.filter((row) => row.id !== selectedCommunityGroupId));
      },
      onLeft: (error) => {
              ...
              },
              {
                ...
                renderCell: (params) => (
                  <GridActionsCell {...params}>
                    ...
                    />
                    {['update-community-group'].some((p) => userPermissions.has(p)) ? (
                      <GridActionsCellItem
                        key="edit"
                        showInMenu
                        icon={<EditRounded />}
                        ...
                        component={Link}
                        // @ts-expect-error Link component requires href prop but it does not exposed as a prop for some reason. Read more on https://github.com/mui/mui-x/issues/9913
                        href={`/community-groups/${params.row.actions.id}/edit`}
                      />
                    ) : null}
                    {['delete-community-group'].some((p) => userPermissions.has(p)) ? (
                      <GridActionsCellItem
                        key="delete"
                        showInMenu
                        icon={<DeleteRounded />}
