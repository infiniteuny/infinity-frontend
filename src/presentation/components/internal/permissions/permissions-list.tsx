...
import { PaginationOptions, Permission } from '@app/domain/entities';
import {
...
export function PermissionsList({ initialPermissions, initialPaginationOptions }: Props) {
  ...
  const handleDeleteClick = (permissionId: string, permissionName?: string) => {
    setSelectedPermissionId(permissionId);
    setSelectedPermissionName(permissionName || null);
  ...
  const handleDeleteAccept = async () => {
    ...
    const result = await deletePermission.execute(selectedPermissionId);
    match(result, {
      onRight: () => {
        setRows((prevRows) => prevRows.filter((row) => row.id !== selectedPermissionId));
      },
      onLeft: (error) => {
              ...
              },
              {
                ...
                renderCell: (params) => (
                  <GridActionsCell {...params}>
                    ...
                    />
                    {['update-permission'].some((p) => userPermissions.has(p)) ? (
                      <GridActionsCellItem
                        key="edit"
                        showInMenu
                        icon={<EditRounded />}
                        ...
                        component={Link}
                        // @ts-expect-error Link component requires href prop but it does not exposed as a prop for some reason. Read more on https://github.com/mui/mui-x/issues/9913
                        href={`/permissions/${params.row.actions.id}/edit`}
                      />
                    ) : null}
                    {['delete-permission'].some((p) => userPermissions.has(p)) ? (
                      <GridActionsCellItem
                        key="delete"
                        showInMenu
                        icon={<DeleteRounded />}
