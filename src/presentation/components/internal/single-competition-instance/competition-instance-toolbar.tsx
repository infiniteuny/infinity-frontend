...
export function CompetitionInstanceToolbar({
  ...
  if (competitionInstanceId) {
    return (
      <Box className="ml-auto">
        {['update-competition'].some((p) => userPermissions.includes(p)) ? (
          <Button
            variant="filled"
            className="ml-4"
            aria-label="Edit competition instance"
            LinkComponent={Link}
            href={`/competitions/${competitionId}/instances/${competitionInstanceId}/edit`}
    ...
    return (
      <Box className="ml-auto flex flex-wrap-reverse justify-end gap-y-2">
        ...
        <Button
          variant="filled"
          className="ml-4"
          aria-label="Save competition instance"
